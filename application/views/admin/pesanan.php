            ...
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <?php 
                            if($row->status == 'requested') { ?>
                                <a href="<?= base_url('admin/Pesanan/updateStatus?status=approved&id='). $row->order_id ?>" class="btn btn-sm btn-info">Terima</a>
                            <?php } else { ?>
                                <a href="<?= base_url('admin/Pesanan/updateStatus?status=requested&id='). $row->order_id ?>" class="btn btn-sm btn-warning">Batalkan</a>
                            <?php } ?>
                            <a href="<?= base_url('admin/Pesanan/delete?id='). $row->order_id ?>" class="btn btn-sm btn-danger" title='Hapus' onclick="if !(confirm('Apakah anda yakin akan menghapus Pesanan ini ??')) {return false; }" >Delete</a>
                    </td> 
                </tr>
                <?php endforeach; ?>
                </tbody>
